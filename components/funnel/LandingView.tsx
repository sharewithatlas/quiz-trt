...
export function LandingView() {
  ...
  const useBlocks = funnel.landingBlocks && funnel.landingBlocks.length > 0;

  // Q1 card as JSX so it can be inserted at the question-anchor point.
  const q1Card = q1 ? (
    <div className="my-8">
    ...
    return (
      <div className="container-prose pt-8 pb-12 md:pt-12">
        ...
        {funnel.landingBlocks!.map((b, i) => {
          if (b.type === 'question-anchor') {
            renderedQ1 = true;
            return <div key={i}>{q1Card}</div>;
          }
  ...
  // Legacy flat-slot rendering for funnels that don't declare landingBlocks.
  return (
    <div>
      ...
      {belowSlots.length > 0 && (
        <section className="container-prose py-12 space-y-6">
          {belowSlots.map((slot) => <Copy key={slot} slot={slot} />)}
